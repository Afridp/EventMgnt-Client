...
function Events() {
  ...
  useEffect(() => {
    const fetchAllBookedEvevts = async () =>{
      try {
        // const res = await getBookedEvents(manager._id)
      } catch (error) {
  ...
  return (
    <>
      ...
      <div className="container mx-auto mt my-10 grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        <a
          href="#"
          className="block rounded-lg p-4 shadow-md border-gray-200 border shadow-indigo-100"
        >
          <img
            alt="Home"
          ...
          <div className="mt-2">
            <dl>
              <div>
                <dd className="text-sm text-gray-500">Concert</dd>
              </div>
            ...
            <div className="mt-6 flex items-center gap-8 text-xs">
              ...
              <div className="sm:inline-flex sm:shrink-0 sm:items-center sm:gap-2">
                <button className="w-[100px] bg-blue-gray-400 h-[40px] ml-14 my-3 flex items-center justify-center rounded-xl cursor-pointer relative overflow-hidden transition-all duration-500 ease-in-out shadow-md hover:scale-105 hover:shadow-lg before:absolute before:top-0 before:-left-full before:w-full before:h-full before:bg-gradient-to-r before:from-[#009b49] before:to-[rgb(105,184,141)] before:transition-all before:duration-500 before:ease-in-out before:z-[-1] before:rounded-xl hover:before:left-0 text-[#fff]">
                  Button
                </button>
              </div>
